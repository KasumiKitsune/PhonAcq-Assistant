...
class VoicebankRecorderPage(QWidget):
    ...
    def __init__(self, parent_window, WORD_LIST_DIR, AUDIO_RECORD_DIR, ToggleSwitchClass, WorkerClass, LoggerClass, icon_manager):
        ...
        self.last_warning_log_time = 0 # [新增] 用于防止日志泛滥
    ...
    def _init_ui(self):
        ...
        self.list_widget = QListWidget(); self.list_widget.setToolTip("当前词表中所有需要录制的词语。\n绿色对勾表示已录制。")
    ...
    def _connect_signals(self):
        ...
        self.record_btn.pressed.connect(self.start_recording); self.record_btn.released.connect(self.stop_recording)
    ...
    def update_list_widget_icons(self):
        ...
        for index in range(self.list_widget.count()):
            ...
            item_data = self.current_word_list[index]
            ...
            if os.path.exists(main_filepath) or (recording_format != 'wav' and os.path.exists(fallback_filepath)):
    ...
    def _format_list_item_text(self, word, ipa):
        ipa_display = f"({ipa})" if ipa else ""; total_weighted_length = len(word) + len(ipa_display) # Simplified
        if total_weighted_length > self.LINE_WIDTH_THRESHOLD / 2 and ipa_display: return f"{word}\n{ipa_display}"
        else: return f"{word} {ipa_display}".strip()
    ...
    def reset_ui(self):
        ...
        self.list_widget.clear(); self.record_btn.setEnabled(False); self.log("请选择一个单词表开始录制。")
    ...
    def start_session(self):
        ...
        try:
            ...
            self.word_list_combo.hide(); self.start_btn.hide(); self.end_session_btn = QPushButton("结束当前会话"); self.end_session_btn.setObjectName("ActionButton_Delete"); self.end_session_btn.clicked.connect(self.end_session); self.update_icons(); self.end_session_btn.show(); self.control_layout.addRow(self.end_session_btn)
    ...
    def update_list_widget(self):
        ...
        for item_data in self.current_word_list:
            display_text = self._format_list_item_text(item_data['word'], item_data['ipa']); self.list_widget.addItem(QListWidgetItem(display_text))
        self.update_list_widget_icons()
        if self.current_word_list and 0 <= current_row < len(self.current_word_list): self.list_widget.setCurrentRow(current_row)
    ...
    def on_recording_saved(self, result):
        ...
        if self.current_word_index + 1 < len(self.current_word_list): self.list_widget.setCurrentRow(self.current_word_index + 1)
        else: 
            all_done = True; recording_format = self.config['audio_settings'].get('recording_format', 'wav').lower()
            for item_data in self.current_word_list:
                main_filepath = os.path.join(self.audio_folder, f"{item_data.get('word')}.{recording_format}"); fallback_filepath = os.path.join(self.audio_folder, f"{item_data.get('word')}.wav")
                if not os.path.exists(main_filepath) and not (recording_format != 'wav' and os.path.exists(fallback_filepath)): all_done = False; break
            if all_done:
    ...
    def save_recording_task(self, worker_instance):
        ...
        try:
            ...
            try: # Nested try for WAV fallback
    ...
    def load_word_list_logic(self,filename):
        ...
        spec=importlib.util.spec_from_file_location("word_list_module",filepath);module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
