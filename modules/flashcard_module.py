...
class FlashcardPage(QWidget):
    ...
    def _init_ui(self):
        ...
        self.mark_mastered_btn = QPushButton("标记/取消掌握"); left_layout.addWidget(self.mark_mastered_btn)
        ...
        self.card_question_area.setStyleSheet("QLabel#FlashcardQuestionArea { font-size: 18pt; font-weight: bold; padding: 10px; min-height: 100px; color: #333; }")
        ...
        self.answer_input = QLineEdit(); self.answer_input.setPlaceholderText("在此输入答案..."); self.answer_submit_btn = QPushButton("提交答案")
        ...
        self.prev_btn = QPushButton("上一个"); self.show_answer_btn = QPushButton("显示/隐藏答案"); self.show_answer_btn.setObjectName("AccentButton"); self.next_btn = QPushButton("下一个"); self.play_audio_btn = QPushButton("播放音频")
        ...
        source_group = QGroupBox("1. 选择词表"); source_layout = QFormLayout(source_group); self.wordlist_combo = QComboBox(); source_layout.addRow("词表文件:", self.wordlist_combo)
    ...
    def update_mode_options(self, index=0):
        ...
        if list_type == "common":
            self.add_mode_button("看词记音 (学习)", "word_to_sound")
            self.add_mode_button("听音记词 (自检)", "sound_to_word")
        elif list_type == "visual":
            self.add_mode_button("看词记图 (学习)", "text_to_image")
            self.add_mode_button("看图记词 (自检)", "image_to_text")
    ...
    def start_session(self):
        ...
        try:
            self.load_and_adapt_data(wordlist_file)
    ...
    def reset_session(self):
        ...
        self.card_question_area.set_pixmap(None); self.card_question_area.setText("请从右侧加载词表开始学习")
    ...
    def load_and_adapt_data(self, filename):
        ...
        if self.current_wordlist_type == "visual":
            for item in module.ITEMS:
                card = {'id': item.get('id', ''), 'image_path': item.get('image_path', ''), 'text': item.get('prompt_text', ''), 'notes': item.get('notes', ''), 'correct_answer': item.get('id', '')}
            ...
            for group in module.WORD_GROUPS:
                for word, value in group.items():
                    ...
                    card = {'id': word, 'text': word, 'notes': ipa, 'correct_answer': word}
                    self.all_loaded_cards.append(card)
        ...
        if self.smart_random_radio.isChecked():
            def get_weight(card):
                ...
                if progress.get("mastered", False):
                    # 对已掌握的词，根据复习间隔计算权重
                    last_viewed_ts = progress.get("last_viewed_ts", 0)
                    next_review_ts = progress.get("next_review_ts", now_ts)
                    if now_ts >= next_review_ts:
                        # 到达或超过复习时间，给予高权重
                        return 50 + (now_ts - next_review_ts) / (3600*24)
                    else:
                        # 未到复习时间，给予极低权重
                        return 0.1
                else:
                    # 对未掌握的词，基于错误和新旧程度计算权重
                    views = progress.get("views", 0)
                    errors = progress.get("errors", 0)
                    # 基础权重100，每次犯错+50，每次浏览权重略微降低（熟悉度）
    ...
    def update_card_display(self):
        ...
        card_content = self.get_card_content(card)
        self.display_content(self.card_question_area, card_content['question'])
        ...
        self.answer_input_widget.setVisible(is_self_test_mode)
        self.show_answer_btn.setVisible(not is_self_test_mode)
    ...
    def get_card_content(self, card):
        """根据模式决定卡片正反面内容"""
        content = {}
        if self.selected_mode_id == "word_to_sound": # 看词记音（学习）
            content['question'], content['answer'] = card['text'], card['notes']
        elif self.selected_mode_id == "sound_to_word": # 听音记词（自检）
            content['question'], content['answer'] = "听音，写出单词...", f"{card['text']}\n({card['notes']})"
        elif self.selected_mode_id == "text_to_image": # 看词记图（学习）
            content['question'], content['answer'] = card['text'], card['image_path']
        elif self.selected_mode_id == "image_to_text": # 看图记词（自检）
            content['question'], content['answer'] = card['image_path'], f"答案: {card['id']}\n({card['text']})"
        return content
    ...
    def display_content(self, widget, content):
        ...
        if is_path:
            ...
            widget.set_pixmap(None); widget.setText(str(content))
    ...
    def toggle_answer(self):
        ...
        if self.is_answer_shown:
            card = self.cards[self.current_card_index]
            answer_data = self.get_card_content(card).get('answer', '')
    ...
    def check_answer(self):
        ...
        if similarity >= 90:
            ...
            # [修改] 调用新的 show_feedback，并设置自动关闭延时
            self.show_feedback("正确!", f"太棒了！下次复习: {next_review.strftime('%Y-%m-%d')}", auto_close_delay=1200)
            auto_advance_delay = 0 # 反馈框关闭后立即跳转
        
        elif similarity >= 70:
            self.update_progress_on_wrong(card.get('id'))
            # [修改] 确保传入的文本是正确的富文本格式
            feedback_text = f"拼写稍有偏差 (相似度: {similarity}%)。<br>正确答案是: <b>{correct_answer}</b>"
            self.show_feedback("基本正确", feedback_text)
            auto_advance_delay = 500 # 用户点击OK后，稍作停留再跳转
        
        else:
            self.update_progress_on_wrong(card.get('id'))
            self.show_feedback("再想想？", f"答案似乎不太对哦 (相似度: {similarity}%)。")
        ...
        self.display_content(self.card_answer_area, self.get_card_content(card).get('answer', ''))
    ...
    def show_feedback(self, title, text, auto_close_delay=-1):
        ...
        # [修改] 使用 setInformativeText 来支持富文本，或者确保传入的 text 是纯文本
        # 为了安全，我们假设 text 可能包含简单的HTML，所以用 setInformativeText
        # 并将主文本设置为空
        msg_box.setText(text) # QMessageBox 现在能更好地处理简单的HTML
        msg_box.setTextFormat(Qt.RichText) # 明确告诉它这是富文本

        # 如果需要自动关闭
        if auto_close_delay > 0:
            msg_box.setStandardButtons(QMessageBox.NoButton) # 无按钮
    ...
    def toggle_mastered_status(self):
        ...
        if new_status:
             progress["level"] = progress.get("level", 0) or 1
             progress["next_review_ts"] = (datetime.now() + timedelta(days=self.get_review_interval(progress["level"]))).timestamp()
        else:
             progress["level"] = 0
        ...
        if self.hide_list_switch.isChecked() and not self.list_widget.isVisible():
            ...
            msg_box = QMessageBox(self); msg_box.setIcon(QMessageBox.Information); msg_box.setText(f"<b>{card_id}</b><br>{status_text}"); msg_box.setWindowTitle("状态更新")
            msg_box.setStandardButtons(QMessageBox.NoButton); msg_box.setWindowModality(Qt.NonModal); msg_box.show()
    ...
    def clear_current_progress(self):
        ...
        if reply == QMessageBox.Yes and os.path.exists(progress_file):
            try:
                os.remove(progress_file); self.progress_data = {}; QMessageBox.information(self, "成功", f"词表 '{wordlist_file}' 的学习记录已清空。")
    ...
    def update_list_widget(self):
        ...
        for i, card in enumerate(self.cards):
            item = QListWidgetItem(card['id']); self.list_widget.addItem(item)
            self.update_list_item_icon(i, self.progress_data.get(card['id'], {}).get("mastered", False))

    def update_list_item_icon(self, row, is_mastered):
        item = self.list_widget.item(row)
        if item: item.setIcon(QApplication.style().standardIcon(QStyle.SP_DialogApplyButton) if is_mastered else QIcon())
    ...
    def get_review_interval(self, level):
        intervals = [1, 2, 4, 7, 15, 30, 60] # in days
    ...
    def update_progress_on_wrong(self, card_id):
        ...
        progress["level"] = 0 # Reset review interval
