...
def apply_sandhi(words, pinyins):
    ...
    new_pinyins = list(pinyins)
    sandhi_flags = [False] * len(pinyins)
    original_tones = [get_tone(p) for p in pinyins]
    ...
    for i in range(len(words)):
        if words[i] == '啊' and i > 0:
            ...
            if new_final:
                new_pinyins[i] = new_final; sandhi_flags[i] = True
    ...
    for i in range(len(words)):
        ...
        if word == '一' and original_tone == '1':
            is_sandhi = False
            if i > 0 and i + 1 < len(words) and words[i-1] == words[i+1]:
                new_pinyins[i] = 'yi5'; is_sandhi = True
            elif i + 1 < len(words):
                next_tone = get_tone(new_pinyins[i+1])
                if next_tone == '4':
                    new_pinyins[i] = 'yi2'; is_sandhi = True
                elif next_tone in ['1', '2', '3']:
                    new_pinyins[i] = 'yi4'; is_sandhi = True
            if is_sandhi: sandhi_flags[i] = True
        
        if word == '不' and original_tone == '4':
            is_sandhi = False
            if i > 0 and i + 1 < len(words) and words[i-1] == words[i+1]:
                new_pinyins[i] = 'bu5'; is_sandhi = True
            elif i + 1 < len(words) and get_tone(new_pinyins[i+1]) == '4':
                new_pinyins[i] = 'bu2'; is_sandhi = True
            if is_sandhi: sandhi_flags[i] = True
    ...
    for i in range(len(temp_pinyins)):
        if get_tone(temp_pinyins[i]) == '3':
            if i + 1 < len(temp_pinyins) and get_tone(temp_pinyins[i+1]) == '3':
                new_pinyins[i] = temp_pinyins[i][:-1] + '2'; sandhi_flags[i] = True
            elif i + 1 < len(temp_pinyins) and get_tone(temp_pinyins[i+1]) != '3':
                new_pinyins[i] = temp_pinyins[i][:-1] + '3s'; sandhi_flags[i] = True
...
def convert_pinyin_to_ipa(pinyin_sandhi_list, scheme):
    ...
    for pinyin, is_sandhi in pinyin_sandhi_list:
        ...
        if is_sandhi and tone_ipa.strip():
            tone_ipa = f'<span style="color: red;">{tone_ipa}</span>'
            
        ipa_syllable = ""
        if pinyin_no_tone in scheme.get("syllables", {}):
            ipa_syllable = scheme["syllables"][pinyin_no_tone]
        elif pinyin_no_tone in scheme.get("syllabic_vowels", {}):
            initial_part = pinyin_no_tone[:-1] if len(pinyin_no_tone) > 1 else ""
            vowel_part = scheme["syllabic_vowels"][pinyin_no_tone]
            initial_ipa = scheme["initials"].get(initial_part, "")
            ipa_syllable = f"{initial_ipa}{vowel_part}"
        else:
            initial = ""; final = pinyin_no_tone
            for i in ['zh','ch','sh','b','p','m','f','d','t','n','l','g','k','h','j','q','x','r','z','c','s','y','w']:
                if pinyin_no_tone.startswith(i):
                    initial = i; final = pinyin_no_tone[len(i):]; break
...
class PinyinToIpaPage(QWidget):
    # ... (init and ui setup remain the same) ...
    ...
    def populate_scheme_table(self, scheme):
        ...
        # --- [新增] 在表格末尾添加来源标注 ---
        last_row = self.scheme_rules_table.rowCount()
        self.scheme_rules_table.insertRow(last_row)
        attribution_item = QTableWidgetItem("以上方案均参考自 UntPhesoca")
        attribution_item.setTextAlignment(Qt.AlignCenter)
        font = attribution_item.font(); font.setItalic(True); attribution_item.setFont(font)
        attribution_item.setForeground(QBrush(QColor("gray")))
        self.scheme_rules_table.setItem(last_row, 0, attribution_item)
        self.scheme_rules_table.setSpan(last_row, 0, 1, 4)
        # --- 结束新增 ---
        
        self.scheme_rules_table.resizeRowsToContents()

    # ... (populate_sandhi_table, on_sandhi_cell_entered, and on_convert_clicked remain the same as the previous correct implementation)
    def populate_sandhi_table(self):
        sandhi_data = [
            ("上声(²¹⁴)的变调", None),
            ("上声 + 上声", "前一个变阳平(³⁵)。例: 你好 nǐ hǎo → ní hǎo"),
            ("上声 + 非上声", "变为半上(²¹)。例: 很好 hěn hǎo"),
            ("“一”和“不”的变调", None),
            ("在去声(⁵¹)前", "一(yī)→yí / 不(bù)→bú。例: 一样 yíyàng / 不怕 búpà"),
            ("在非去声前", "一(yī)→yì。例: 一天 yìtiān (不(bù)声调不变)"),
            ("在重叠词中", "读轻声。例: 看一看 kàn yi kan / 好不好 hǎo bu hǎo"),
            ("“啊”的变读", None),
            ("前为a,o,e,i,ü", "啊(a) → 呀(ya)"),
            ("前为u(ao,iao)", "啊(a) → 哇(wa)"),
            ("前为n", "啊(a) → 哪(na)"),
            ("前为ng", "啊(a) → 啊(nga)"),
        ]
