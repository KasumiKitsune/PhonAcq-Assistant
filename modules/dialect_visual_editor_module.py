...
def create_page(parent_window, word_list_dir_visual, ToggleSwitchClass):
    global WORD_LIST_DIR_FOR_DIALECT_VISUAL
    WORD_LIST_DIR_FOR_DIALECT_VISUAL = word_list_dir_visual
    return DialectVisualEditorPage(parent_window, ToggleSwitchClass)

# [移除] 不再需要 PathTableWidgetItem
...
class ChangeCellCommand(QUndoCommand):
    ...
    def _set_text(self, text):
        if self.col == 1: # 如果是图片路径列
            item = QTableWidgetItem()
            # [修改] 同时设置 DisplayRole 和 EditRole
...
class DialectVisualEditorPage(QWidget):
    ...
    def _init_ui(self):
        ...
        self.table_widget.setColumnCount(4); self.table_widget.setHorizontalHeaderLabels(["项目ID (必须唯一)", "图片文件", "提示文字 (可选)", "备注 (可选)"])
    ...
    def clear_selection_contents(self):
        ...
        for item in selected_items:
            ...
            if old_text:
                cmd = ChangeCellCommand(self, item.row(), item.column(), old_text, "", "清空单元格"); self.undo_stack.push(cmd)
    ...
    def copy_selection(self):
        ...
        # [修改] 从 EditRole 获取数据用于复制
        table_str = "\n".join(["\t".join([self.table_widget.item(r, c).data(Qt.EditRole) if self.table_widget.item(r, c) else "" for c in cols]) for r in rows])
        ...
        for i, row in enumerate(rows):
            ...
            for j, cell_text in enumerate(cells):
                ...
                if target_row < self.table_widget.rowCount() and target_col < self.table_widget.columnCount():
                    ...
                    old_text = item.data(Qt.EditRole) if item else ""
    ...
    def auto_detect_images(self):
        ...
        is_smart = self.smart_detect_switch.isChecked(); detected_count = 0
    ...
    def _write_to_file(self, filepath):
        ...
        for row in range(self.table_widget.rowCount()):
            ...
            item_data['image_path'] = image_path.strip()
