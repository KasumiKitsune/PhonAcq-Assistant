...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QListWidget,
                             ...
                             QTableWidgetItem, QHeaderView, QCheckBox, QShortcut,
                             QUndoStack, QUndoCommand, QApplication, QMenu, QAction)
...
# ===== MODIFIED: 增强 Undo/Redo 命令类 =====
class ChangeCellCommand(QUndoCommand):
    """一个封装了单元格内容修改的命令，用于撤销/重做。"""
...
class RowOperationCommand(QUndoCommand):
    """一个封装了行增加、删除、移动的命令，用于撤销/重做。"""
    # ===== MODIFIED: 构造函数接收 editor 实例 =====
...
class DialectVisualEditorPage(QWidget):
    def __init__(self, parent_window):
        ...
        left_panel = QWidget(); left_layout = QVBoxLayout(left_panel); left_panel.setFixedWidth(280)
        ...
        self.table_widget.setHorizontalHeaderLabels(["项目ID (必须唯一)", "图片文件路径 (可选)", "提示文字 (可选)", "备注 (可选)"])
        ...
        self.new_btn.clicked.connect(self.new_wordlist); self.save_btn.clicked.connect(self.save_wordlist)
        self.save_as_btn.clicked.connect(self.save_wordlist_as); self.add_row_btn.clicked.connect(lambda: self.add_row())
        self.remove_row_btn.clicked.connect(self.remove_row); self.auto_detect_btn.clicked.connect(self.auto_detect_images)
    ...
    def setup_shortcuts(self):
        ...
        QShortcut(QKeySequence(Qt.ALT | Qt.Key_Up), self, lambda: self.move_rows(-1))
        QShortcut(QKeySequence(Qt.ALT | Qt.Key_Down), self, lambda: self.move_rows(1))
        QShortcut(QKeySequence(Qt.CTRL | Qt.Key_Plus), self, lambda: self.add_row())
        QShortcut(QKeySequence(Qt.CTRL | Qt.Key_Equal), self, lambda: self.add_row())
    ...
    def on_item_changed_for_undo(self, item):
        if self.old_text_before_edit is not None:
            new_text = item.text()
            if self.old_text_before_edit != new_text:
                # ===== MODIFIED: 传递 self 给命令构造函数 =====
                command = ChangeCellCommand(self, item.row(), item.column(), self.old_text_before_edit, new_text, f"修改单元格")
    ...
    def validate_id_uniqueness(self, changed_item):
        ...
        if old_text and old_text in self.id_widgets:
            if changed_item in self.id_widgets[old_text]:
                self.id_widgets[old_text].remove(changed_item)
            if not self.id_widgets[old_text]:
                del self.id_widgets[old_text]
            if old_text in self.id_widgets and len(self.id_widgets[old_text]) == 1:
                self.id_widgets[old_text][0].setBackground(QBrush(Qt.white))

        new_text = changed_item.text().strip()
        if not new_text:
            changed_item.setBackground(QBrush(Qt.white))
            return
            
        if new_text not in self.id_widgets:
            self.id_widgets[new_text] = []
        if changed_item not in self.id_widgets[new_text]:
            self.id_widgets[new_text].append(changed_item)

        if len(self.id_widgets[new_text]) > 1:
            for item in self.id_widgets[new_text]:
    ...
    def show_context_menu(self, position):
        ...
        menu.addSeparator()
        duplicate_action = menu.addAction("创建副本/重制行 (Ctrl+D)")
        menu.addSeparator()
        add_row_action = menu.addAction("在下方插入新行")
        remove_row_action = menu.addAction("删除选中行")
        menu.addSeparator()
        clear_contents_action = menu.addAction("清空内容 (Delete)")

        if not selection:
            cut_action.setEnabled(False); copy_action.setEnabled(False)
    ...
    def clear_selection_contents(self):
        ...
        for item in selected_items:
            if item.text():
                cmd = ChangeCellCommand(self, item.row(), item.column(), item.text(), "", "清空单元格")
        ...
        for i, row in enumerate(rows):
            ...
            for j, cell_text in enumerate(cells):
                ...
                if target_row < self.table_widget.rowCount() and target_col < self.table_widget.columnCount():
                    ...
                    cmd = ChangeCellCommand(self, target_row, target_col, old_text, cell_text, "粘贴单元格")
                    self.undo_stack.push(cmd)
        self.undo_stack.endMacro()
    ...
    def move_rows(self, offset):
        ...
        if offset == -1 and selected_rows[0] == 0: return
        if offset == 1 and selected_rows[-1] == self.table_widget.rowCount() - 1: return

        start_row = selected_rows[0]
        rows_data = self._get_rows_data(selected_rows)
        ...
        for i in range(len(selected_rows)):
            self.table_widget.selectRow(new_start_row + i)
            
    def add_row(self, at_row=None):
        if at_row is None:
            at_row = self.table_widget.rowCount()
        
        cmd = RowOperationCommand(self, at_row, [["", "", "", ""]], 'add', description="添加新行")
        self.undo_stack.push(cmd)
        # 确保item被创建后再滚动
        QApplication.processEvents()
        item_to_scroll = self.table_widget.item(at_row, 0)
        if item_to_scroll:
            self.table_widget.scrollToItem(item_to_scroll, QTableWidget.ScrollHint.EnsureVisible)
    ...
    def auto_detect_images(self):
        if not self.current_wordlist_path:
            QMessageBox.warning(self, "操作无效", "请先加载一个图文词表。")
            return
        wordlist_dir = os.path.dirname(self.current_wordlist_path)
        wordlist_name_no_ext = os.path.splitext(os.path.basename(self.current_wordlist_path))[0]
        image_folder_path = os.path.join(wordlist_dir, wordlist_name_no_ext)
        if not os.path.isdir(image_folder_path):
            QMessageBox.information(self, "提示", f"未找到对应的图片文件夹，请确保它存在于:\n{image_folder_path}")
            return
            
        is_smart_mode = self.smart_detect_checkbox.isChecked()
        detected_count = 0
        image_files = [f for f in os.listdir(image_folder_path) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))]
        normalized_image_map = {self._normalize_string(os.path.splitext(f)[0]): f for f in image_files}
        used_images = set()
                ...
                if id_item and id_item.text().strip() and (not image_item or not image_item.text().strip()):
                    ...
                    if not is_smart_mode:
                        ...
                        for ext in supported_extensions:
                            potential_image_name = item_id + ext
                            if os.path.exists(os.path.join(image_folder_path, potential_image_name)):
                                best_match_filename = potential_image_name
                                break
                    else:
                        normalized_id = self._normalize_string(item_id)
                        best_score = 0; best_match_normalized_name = None
                        for norm_name in normalized_image_map.keys():
                            if norm_name in used_images: continue
                            score = fuzz.ratio(normalized_id, norm_name)
                            if score > best_score: best_score = score; best_match_normalized_name = norm_name
                        similarity_threshold = 70 
                        if best_score >= similarity_threshold and best_match_normalized_name:
                            best_match_filename = normalized_image_map[best_match_normalized_name]
                            used_images.add(best_match_normalized_name)
                    if best_match_filename:
                        display_path = f"{wordlist_name_no_ext}/{best_match_filename}".replace("\\", "/")
                        old_text = image_item.text() if image_item else ""
                        command = ChangeCellCommand(self, row, 1, old_text, display_path, "自动填充图片路径")
                        self.undo_stack.push(command)
                        detected_count += 1
        finally:
            self.undo_stack.endMacro()

        if detected_count > 0: QMessageBox.information(self, "检测完成", f"成功检测并填充了 {detected_count} 个图片文件。")
        else: QMessageBox.information(self, "检测完成", "没有找到可以自动填充的新图片文件。")

    def on_cell_double_clicked(self, row, column):
        if column != 1: return
        if not self.current_wordlist_path:
            QMessageBox.warning(self, "操作无效", "请先加载或保存一个词表文件，以确定图片相对路径的基准。")
            return
        wordlist_dir = os.path.dirname(self.current_wordlist_path)
        wordlist_name_no_ext = os.path.splitext(os.path.basename(self.current_wordlist_path))[0]
        image_folder_path = os.path.join(wordlist_dir, wordlist_name_no_ext)
        if not os.path.isdir(image_folder_path): os.makedirs(image_folder_path)
        filepath, _ = QFileDialog.getOpenFileName(self, "选择图片文件", image_folder_path, "图片文件 (*.png *.jpg *.jpeg *.bmp *.gif)")
        if filepath:
            ...
            command = ChangeCellCommand(self, row, 1, old_text, display_path, "选择图片文件")
            self.undo_stack.push(command)
    ...
    def on_file_selected(self, current, previous):
        if not self.undo_stack.isClean() and previous:
            reply = QMessageBox.question(self, "未保存的更改", "您有未保存的更改，确定要切换吗？更改将会丢失。", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def _write_to_file(self, filepath):
        ...
        for row in range(self.table_widget.rowCount()):
            ...
            image_path_item = self.table_widget.item(row, 1); item_data['image_path'] = image_path_item.text().strip() if image_path_item else ''
            prompt_text_item = self.table_widget.item(row, 2); item_data['prompt_text'] = prompt_text_item.text().strip() if prompt_text_item else ''
            notes_item = self.table_widget.item(row, 3); item_data['notes'] = notes_item.text().strip() if notes_item else ''
            items_list.append(item_data)

        py_code = f"# Auto-generated by PhonAcq Assistant Dialect Visual Editor\n"; py_code += f"# Save date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        py_code += "# WORDLIST_NAME = \"\"\n"; py_code += "# WORDLIST_DESCRIPTION = \"\"\n\n"; py_code += "ITEMS = [\n"
