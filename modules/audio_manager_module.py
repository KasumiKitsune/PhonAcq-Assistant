...
class AudioManagerPage(QWidget):
    def __init__(self, parent_window, config, base_path, results_dir, audio_record_dir):
        ...
        self.audio_table_widget.customContextMenuRequested.connect(self.open_file_context_menu)
        ...
        playback_layout = QHBoxLayout(playback_panel); playback_layout.setContentsMargins(0, 5, 0, 5)
        self.play_pause_btn = QPushButton("播放"); self.play_pause_btn.setMinimumWidth(80)
        self.playback_slider = QSlider(Qt.Horizontal)
        self.duration_label = QLabel("00:00.0 / 00:00.0")
        
        self.volume_label = QLabel("音量:")
        self.volume_slider = QSlider(Qt.Horizontal); self.volume_slider.setFixedWidth(100)
        self.volume_percent_label = QLabel("75%")
        
        self.volume_slider.setRange(0, 100); self.volume_slider.setValue(75)
        self.player.setVolume(75)
        
        playback_layout.addWidget(self.play_pause_btn); playback_layout.addWidget(self.playback_slider)
        playback_layout.addWidget(self.duration_label); playback_layout.addSpacing(20)
        playback_layout.addWidget(self.volume_label); playback_layout.addWidget(self.volume_slider); playback_layout.addWidget(self.volume_percent_label)
        ...
        # ===== 修改/MODIFIED: 播放按钮的点击事件现在调用一个新的方法 =====
    ...
    def update_table_row(self, row, filepath):
        ...
        # ===== 修改/MODIFIED: 将删除按钮改为新的链接式按钮 =====
        delete_btn = QPushButton("删除")
        delete_btn.setObjectName("LinkButton") # 应用新的QSS样式ID
        delete_btn.setToolTip("删除此文件")
        delete_btn.setCursor(Qt.PointingHandCursor) # 设置手型光标
    ...
    def populate_accent_list(self):
        current_selection_text = self.accent_list_widget.currentItem().text() if self.accent_list_widget.currentItem() else None
        self.accent_list_widget.clear()
        if not os.path.exists(self.RESULTS_DIR): os.makedirs(self.RESULTS_DIR); return
        sessions = [d for d in os.listdir(self.RESULTS_DIR) if os.path.isdir(os.path.join(self.RESULTS_DIR, d)) and os.path.exists(os.path.join(self.RESULTS_DIR, d, 'log.txt'))]
        sessions.sort(key=lambda s: os.path.getmtime(os.path.join(self.RESULTS_DIR, s)), reverse=True)
        self.accent_list_widget.addItems(sessions)
        if current_selection_text:
            items = self.accent_list_widget.findItems(current_selection_text, Qt.MatchFixedString)
            if items: self.accent_list_widget.setCurrentItem(items[0])

    def populate_voicebank_list(self):
        current_selection_text = self.voicebank_list_widget.currentItem().text() if self.voicebank_list_widget.currentItem() else None
        self.voicebank_list_widget.clear()
        if not os.path.exists(self.AUDIO_RECORD_DIR): os.makedirs(self.AUDIO_RECORD_DIR); return
        voicebanks = [d for d in os.listdir(self.AUDIO_RECORD_DIR) if os.path.isdir(os.path.join(self.AUDIO_RECORD_DIR, d))]
        voicebanks.sort(key=lambda s: os.path.getmtime(os.path.join(self.AUDIO_RECORD_DIR, s)), reverse=True)
        self.voicebank_list_widget.addItems(voicebanks)
        if current_selection_text:
            items = self.voicebank_list_widget.findItems(current_selection_text, Qt.MatchFixedString)
    ...
    def rename_selected_file(self):
        ...
        if ok and new_basename and new_basename != old_basename:
            new_filename = new_basename.strip() + ext
            new_filepath = os.path.join(self.current_session_path, new_filename)
            if os.path.exists(new_filepath): QMessageBox.warning(self, "错误", "文件名已存在。"); return
            try:
                os.rename(old_filepath, new_filepath)
                self.update_table_row(row, new_filepath)
    ...
    def open_folder_context_menu(self, list_widget, position, data_type):
        ...
        if action == open_action:
            folder_path = os.path.join(base_dir, selected_items[0].text())
            self.open_in_explorer(folder_path)
        elif action == rename_action:
            self.rename_folder(list_widget, selected_items[0], base_dir)
        elif action == delete_action:
            self.delete_folders(list_widget, selected_items, base_dir)
    ...
    def rename_folder(self, list_widget, item, base_dir):
        ...
        if ok and new_name and new_name != old_name:
            ...
            try:
                os.rename(old_path, new_path); item.setText(new_name)
            except Exception as e: QMessageBox.critical(self, "错误", f"重命名失败: {e}")
        ...
        if reply == QMessageBox.Yes:
            for item in items:
                ...
                try: shutil.rmtree(folder_path)
                    ...
                    QMessageBox.critical(self, "删除失败", f"删除文件夹 '{item.text()}' 时出错: {e}"); break
    ...
    def open_file_context_menu(self, position):
        ...
        if action == play_action: self.play_selected_item(row)
            ...
            self.open_in_explorer(self.current_session_path)
    ...
    def play_selected_item(self, row):
        ...
        if filepath and os.path.exists(filepath):
            if self.player.media().canonicalUrl() == QUrl.fromLocalFile(filepath):
    ...
    def update_playback_duration(self, duration):
        ...
        self.duration_label.setText(f"0:00.00 / {self.format_time(duration)}")
    ...
    def reset_player(self):
        self.player.stop(); self.playback_slider.setValue(0); self.duration_label.setText("0:00.00 / 0:00.00")