...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QListWidget,
                             ...
                             QUndoStack, QUndoCommand, QApplication, QMenu, QSizePolicy)
# ===== MODIFIED: 导入 QEvent =====
...
class WordlistEditorPage(QWidget):
    ...
    def eventFilter(self, source, event):
        if source is self.table_widget.viewport() and event.type() == QEvent.Wheel:
            ...
            if item and item.column() == 0:
                try:
                    ...
                    current_value = int(old_value_str)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Delete or event.key() == Qt.Key_Backspace:
            if self.table_widget.selectedItems(): self.clear_selection_contents(); event.accept()
    ...
    def copy_selection(self):
        ...
        for r in rows:
            ...
            for c in cols:
                if c == 3:
                    combo = self.table_widget.cellWidget(r, c)
                    row_list.append(combo.currentData() if combo else "")
                else:
                    item = self.table_widget.item(r, c); row_list.append(item.text() if item else "")
            ...
            for j, cell_text in enumerate(cells):
                ...
                if target_row < self.table_widget.rowCount() and target_col < self.table_widget.columnCount():
                    if target_col == 3:
                        ...
                        if combo:
                            old_lang = combo.currentData()
                            if old_lang != cell_text:
                                cmd = WordlistChangeLanguageCommand(self, target_row, old_lang, cell_text, "粘贴语言")
                                self.undo_stack.push(cmd)
                    else:
                        item = self.table_widget.item(target_row, target_col); old_text = ""
                        if not item: item = QTableWidgetItem(); self.table_widget.setItem(target_row, target_col, item)
                        else: old_text = item.text()
    ...
    def add_row(self, at_row=None):
        ...
        last_group = "1"
        if at_row > 0:
            last_group_item = self.table_widget.item(at_row - 1, 0)
            if last_group_item and last_group_item.text().isdigit():
                last_group = last_group_item.text()
        cmd = WordlistRowOperationCommand(self, at_row, [[last_group, "", "", ""]], 'add', description="添加新行")
        self.undo_stack.push(cmd)
        QApplication.processEvents()
        item_to_scroll = self.table_widget.item(at_row, 0)
        if item_to_scroll: self.table_widget.scrollToItem(item_to_scroll, QTableWidget.ScrollHint.EnsureVisible)
        ...
        try:
            ...
            for i, group in enumerate(word_groups, 1):
                for word, value in group.items():
                    ...
                    self.populate_row(row, [str(i), word, ipa, lang_code]); row += 1
