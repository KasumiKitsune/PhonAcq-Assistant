...
**PhonAcq (风纳客)** 是一款专为语言学研究者、教师和学习者设计的集成化桌面应用程序。它致力于将繁琐的语音数据采集、语料构建、数据管理乃至后续的学习复习流程变得简单、高效和规范化。
...
1.  **准备语料 (采集或学习前)**:
    *   **方法一 (对于初学者):** 前往 `数据准备 -> Excel 转换器`，下载模板，并且照着模板填写自己实验所需的词条，然后将填写好的 `.xlsx` 文件转换为程序可用的 `.json` 词表文件。这是与不熟悉代码的协作者共享和编辑词表的最佳方式。
    *   **方法二 (推荐):** 
        *   **标准词表**: 前往 `数据准备 -> 通用词表编辑器` 直接创建或编辑用于“标准朗读采集”的 `.json` 文件。
        *   **图文词表**: 前往 `数据准备 -> 图文词表编辑器` 创建或编辑用于“图文采集”的 `.json` 文件。
2.  **生成提示音 (可选但推荐)**:
    *   **真人语音包**: 前往 `数据采集 -> 提示音录制`，为您的标准词表录制一套高质量的真人提示音。这是保证音质和一致性的最佳选择。
    *   **TTS批量生成**: 选择词表，点击“开始新会话”并且在弹出的菜单选择“使用TTS提示音”，软件将会根据词表自动生成提示音。您也可以前往 `实用工具 -> TTS 工具`，加载您的词表，批量生成合成语音。您可以选择将其输出到速记卡模块的专用音频目录。
    * 注：TTS是Text to speech的缩写，意为文字转语音。本程序采用的是gTTS，也就是谷歌提供的免费TTS工具，虽然可以保证读音准确，但是不够自然，有条件的可以去使用Google Cloud Text-to-Speech AI。
3.  **开始采集 (研究)**:
    *   **标准朗读采集**: 前往 `数据采集 -> 标准朗读采集`，进行标准的跟读实验。
    *   **方言图文采集**: 前往 `数据采集 -> 看图说话采集`，进行看图说话式的田野调查。
4.  **管理与编辑数据**:
    *   前往 `语料管理 -> 音频数据管理器`，在这里您可以浏览、试听、重命名、删除所有已录制的音频数据。更重要的是，您还可以使用内置的**波形编辑器**对音频进行**裁切**和**连接**。
    *   前往 `语料管理 -> 日志查看器`，审查每次采集会话的详细过程日志，或切换到“速记卡模式”分析学习进度。日志以纯文本形式记录在会话的文件夹内，因此不会占用太多空间，我们推荐保留日志，但是您可以在设置中取消勾选“启用详细日志记录”。
5.  **开始学习 (复习)**:
    *   前往 `实用工具 -> 速记卡`，加载标准或图文词表，或者用推荐的fdeck词表，选择学习或自检模式，开始高效记忆。不过这个功能不属于实验的一环，可以在设置的“模块管理”选项卡中右键该模组，将其禁用（推荐）或卸载。
...
*   **会话准备**: 您需要先选择一个 `.json` 格式的**标准词表**，并为被试者命名。系统会自动检查并生成缺失的TTS提示音，确保实验的顺利进行。
...
* 注：创建.fdeck速记卡文件需要额外到插件市场获取“速记卡管理器”。
...
####  其他
右键子标签页后可以看到菜单，有**刷新此标签页**、**在新窗口中打开**、**模块设置**、**性能监视**、**设为启动页**等项。

*  **刷新此标签页**：对该标签页的所有状态进行刷新，相当于恢复到程序刚打开时的样子。

* **在新窗口中打开**：将该标签页变为独立的窗口，即使将主程序给关掉，独立窗口仍然会保留。但是无法使用插件。

* **模块设置**：部分模组有单独的设置页面，双击标签页或者右键点按设置后弹出弹窗，对该标签页进行高级设置。例如您可以在标准朗读采集的设置页面中对智能跟读模式进行设置，或者调整波形图的显隐和音量计的刷新率以优化性能。

* **性能监视**：对程序和模组的占用进行查看。此功能只有打开后才会开始监视，因此不会占用额外的性能。您可以在`设置-模块管理`中禁用此功能。

* **设为启动页**：设置后，程序启动时将以该页作为首页。
