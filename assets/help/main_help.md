...
*   **词表与资源**: 此功能使用专属的“图文词表”。所有图片资源都应放在与图文词表 `.py` 文件同名的文件夹下。

### 速记卡 (Flashcard)
**路径**: `实用工具 -> 速记卡`

一个功能强大的学习工具，它将您的词表（标准或图文）变成交互式的电子卡片，并采用基于遗忘曲线的智能算法来优化您的学习和记忆效率。

*   **学习与自检分离**:
    *   **学习模式** (如“看词记音”): 您可以自由浏览卡片正反面，主动跟读和记忆，此模式下没有答案输入框。
    *   **自检模式** (如“听音记词”): 程序会隐藏答案，要求您在输入框中作答。提交后，系统会根据答案的相似度给出反馈，并自动记录您的掌握情况。
*   **智能随机顺序**:
    *   这是推荐的默认模式。它会优先展示您**不熟悉、经常答错、或很久没复习**的词条。
    *   对于已掌握的词条，它会根据艾宾浩斯遗忘曲线原理，在未来的某个时间点（如1天、3天、7天后）再次呈现给您进行复习。
    *   多次复习都正确的词条出现的频率会逐渐降低，让您能专注于难点。
*   **进度管理**:
    *   程序会为**每个词表**自动创建一个学习进度文件。
    *   您可以随时“标记/取消掌握”一个词条。
    *   如果需要重新开始学习一个词表，可以使用“清空当前词表学习记录”按钮。
...
*   **文件选择**: 双击“图片文件”单元格，可以直接弹出文件管理器让您选择图片。路径会自动处理为相对于词表图片文件夹的相对路径。
